...
def favicon():
    # 返回图标
    return os.path.join(os.path.dirname(__file__), 'static', 'My_hacker_dream.png')
...
def add_target():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not domain:
                domain = target_ip.replace(
                    'https://', '').replace('http://', '')
                logger.debug(f"自動提取domain: {domain}")

            current_app.logger.info(
                f"嘗試添加新目標: target_ip={target_ip}, domain={domain}")
            ...
            current_app.logger.info(
                f"✅ 成功添加目標: ID={new_target.id}, target_ip={target_ip}")
            ...
            current_app.logger.error(
                f"詳細信息: target_ip={target_ip}, domain={domain}")
...
def api_delete_target(target_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"❌ API 刪除目標失敗: {str(e)}, target_id={target_id}")
