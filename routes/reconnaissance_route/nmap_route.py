...
def nmap_scan(target_id):
    ...
    try:
        ...
        current_app.logger.debug(
            f"正在访问 nmap 路由，目标 ID: {target_id} {client_ip}")
        logger.info(f"收到 Nmap 掃描請求，目標ID: {target_id}, 客戶端IP: {client_ip}")
        ...
        # 启动扫描
        scan_id = start_nmap_scan(
            target_id, target_data, current_app._get_current_object())

        logger.info(f"Nmap 掃描已啟動，掃描ID: {scan_id}")
        ...
        logger.error(f"啟動 Nmap 掃描時出錯: {str(e)}")
        return jsonify({
            'success': False,
...
def get_nmap_result(target_id):
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info(f"獲取 Nmap 掃描結果，目標ID: {target_id}")
        ...
        if not nmap_result:
            logger.warning(
                f"未找到 Nmap 掃描結果，目標ID: {target_id}, 掃描類型: {scan_type}")
            return jsonify({
                'success': False,
                'message': '未找到扫描结果',
                'status': 'not_found'
        ...
        logger.info(
            f"成功獲取 Nmap 掃描結果，找到 {len(result_dict.get('ports', []))} 個端口")
        ...
        logger.error(f"獲取 Nmap 掃描結果時出錯: {str(e)}")
        return jsonify({
            'success': False,
...
def get_scan_status(target_id):
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info(f"獲取 Nmap 掃描狀態，目標ID: {target_id}")
        ...
        logger.error(f"獲取 Nmap 掃描狀態時出錯: {str(e)}")
        return jsonify({
            'success': False,
...
def get_scan_history(target_id):
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info(f"獲取 Nmap 掃描歷史，目標ID: {target_id}")

        # 获取所有扫描结果
        results = nmap_Result.query.filter_by(target_id=target_id).order_by(
            nmap_Result.scan_time.desc()).all()
        ...
        logger.info(f"獲取到 {len(history)} 條掃描歷史記錄")
        ...
        logger.error(f"獲取 Nmap 掃描歷史時出錯: {str(e)}")
        return jsonify({
            'success': False,
...
def nmap_dashboard():
    ...
    try:
        ...
        logger.error(f"載入 Nmap 掃描器界面時出錯: {str(e)}")
