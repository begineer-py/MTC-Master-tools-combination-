...
def flaresolverr_solve():
    ...
    try:
        logger.info("收到 FlareSolverr 解析請求")
        # 获取目标ID和URL
        ...
        if not url:
            logger.warning("FlareSolverr 請求缺少URL參數")
            return jsonify({
                'success': False,
                'message': '缺少URL参数'
            }), 400

        logger.info(f"開始處理 FlareSolverr 請求，URL: {url}")
        ...
        if response.status_code != 200:
            logger.error(f"FlareSolverr 服務返回錯誤代碼: {response.status_code}")
            return jsonify({
                'success': False,
        ...
        if result.get('status') != 'ok':
            logger.error(f"FlareSolverr 解析失敗: {result.get('message', '未知錯誤')}")
            return jsonify({
                'success': False,
        ...
        logger.info("FlareSolverr 解析成功")
        return jsonify(solution)

    except requests.RequestException as e:
        current_app.logger.error(f"FlareSolverr请求错误: {str(e)}")
        logger.error(f"FlareSolverr 請求錯誤: {str(e)}")
        return jsonify({
            'success': False,
        ...
        logger.error(f"FlareSolverr 解析錯誤: {str(e)}")
        return jsonify({
            'success': False,
...
def start_flaresolverr():
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info("收到啟動 FlareSolverr 服務請求")
        result = flaresolverr_manager.start_flaresolverr()
        status_code = 200 if result['success'] else 500

        if result['success']:
            logger.info("FlareSolverr 服務啟動成功")
        else:
            logger.error(
                f"FlareSolverr 服務啟動失敗: {result.get('message', '未知錯誤')}")

        return jsonify(result), status_code
    except Exception as e:
        current_app.logger.error(f"啟動 FlareSolverr 錯誤: {str(e)}")
        logger.error(f"啟動 FlareSolverr 錯誤: {str(e)}")
        return jsonify({
            'success': False,
...
def stop_flaresolverr():
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info("收到停止 FlareSolverr 服務請求")
        result = flaresolverr_manager.stop_flaresolverr()
        status_code = 200 if result['success'] else 500

        if result['success']:
            logger.info("FlareSolverr 服務停止成功")
        else:
            logger.error(
                f"FlareSolverr 服務停止失敗: {result.get('message', '未知錯誤')}")

        return jsonify(result), status_code
    except Exception as e:
        current_app.logger.error(f"停止 FlareSolverr 錯誤: {str(e)}")
        logger.error(f"停止 FlareSolverr 錯誤: {str(e)}")
        return jsonify({
            'success': False,
...
def restart_flaresolverr():
    ...
    logger = LogConfig.get_context_logger()
    try:
        logger.info("收到重啟 FlareSolverr 服務請求")
        result = flaresolverr_manager.restart_flaresolverr()
        status_code = 200 if result['success'] else 500

        if result['success']:
            logger.info("FlareSolverr 服務重啟成功")
        else:
            logger.error(
                f"FlareSolverr 服務重啟失敗: {result.get('message', '未知錯誤')}")

        return jsonify(result), status_code
    except Exception as e:
        current_app.logger.error(f"重啟 FlareSolverr 錯誤: {str(e)}")
        logger.error(f"重啟 FlareSolverr 錯誤: {str(e)}")
        return jsonify({
            'success': False,
...
def auto_start_flaresolverr():
    ...
    try:
        ...
        if result['success']:
            logger.info("FlareSolverr 自動啟動成功")
        else:
            logger.error(
                f"FlareSolverr 自動啟動失敗: {result.get('message', '未知錯誤')}")

        return jsonify(result), status_code
    except Exception as e:
        current_app.logger.error(f"自動啟動 FlareSolverr 錯誤: {str(e)}")
        logger.error(f"自動啟動 FlareSolverr 錯誤: {str(e)}")
        return jsonify({
            'success': False,
...
def flaresolverr_dashboard():
    """FlareSolverr 管理界面"""
    logger = LogConfig.get_context_logger()
    logger.info("載入 FlareSolverr 管理界面")
...
def flaresolverr_help():
    """FlareSolverr 使用說明"""
    logger = LogConfig.get_context_logger()
    logger.info("獲取 FlareSolverr 使用說明")
    help_info = {
        'success': True,
        'service_info': {
            'name': 'FlareSolverr',
