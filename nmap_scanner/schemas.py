...
class NmapScanSchema(Schema):
    id: int
    ip_id: int
    ip_address: str # 操！直接返回 IP 地址，前端不用再查一次
    scan_type: str
    ...
    # 操！這裡關聯的是本次掃描「更新」的端口，不是 IP 的所有端口
    ports_updated: List[PortSchema] = []

# 掃描任務列表的概覽
class NmapScanOverviewSchema(Schema):
    id: int
    ip_id: int
    ip_address: str
    target_domain: str # 操！在列表裡顯示根域名，一目了然
    status: str
    started_at: datetime
    completed_at: Optional[datetime] = None
    ports_updated_count: int # 本次掃描更新了多少端口
