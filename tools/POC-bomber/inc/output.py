...
def output_result(result):
    ...
    try:
        if not result.get("name"):
            log_info(f(r"检测超时 {percent_str}\[[{timeout_url_color}]{current_target}[/]]  script: {current_script} "))
            return False
        if result['vulnerable']:
            ...
            log_info(f(r"正在检测 {percent_str}\[[{url_color}]{current_target.rstrip('/')}[/{url_color}]]  poc: {result['name']}"))
            return False
    except:
