...
def fetch_url_content(url,proxy):
    ...
    Returns a response object with properly encoded text content.
    """
    ...
    for i in range(MAX_RETRIES):
        ...
        try:
            ...
            if isinstance(response.content, bytes):
                try:
                    response._content = response.content.decode(response.encoding, errors='ignore').encode()
                except UnicodeError:
                    response._content = response.content.decode('utf-8', errors='ignore').encode()
            
            return response
        except (requests.exceptions.RequestException, ValueError) as e:
            logging.warning(f"Error fetching URL {url}: {str(e)}. Retrying in 5 seconds...")
            time.sleep(5)
